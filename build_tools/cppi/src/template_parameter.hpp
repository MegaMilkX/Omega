...
class template_parameter {
    ...
    template_parameter(e_template_parameter_kind kind, TYPE_ID default_arg, bool is_pack)
        : kind(kind), tid(default_arg), is_pack_param(is_pack) {}
    template_parameter(e_template_parameter_kind kind, TYPE_ID tid, const eval_value& default_arg, bool is_pack)
        : kind(kind), tid(tid), non_type_default_arg(default_arg), is_pack_param(is_pack) {}

public:
    static template_parameter make_type(TYPE_ID default_arg, bool is_pack) {
        return template_parameter(e_template_parameter_type, default_arg, is_pack);
    }
    static template_parameter make_non_type(TYPE_ID tid, const eval_value& default_arg, bool is_pack) {
    ...
    template_argument get_default_arg(int dbg_arg_index = 0) const {
        assert(has_default_arg());
        if (kind == e_template_parameter_type) {
            return template_argument::make_type(tid, dbg_arg_index);
        } else if (kind == e_template_parameter_non_type) {
            return template_argument::make_non_type(non_type_default_arg, dbg_arg_index);
        } else if (kind == e_template_parameter_template) {
            assert(false);
            return template_argument::make_empty(dbg_arg_index);
        }
        assert(false);
        return template_argument::make_empty(dbg_arg_index);
    }
