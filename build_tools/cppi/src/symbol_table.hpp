...
public:
    ...
    void print_all() {
        for (auto& kv : symbols) {
            dbg_printf_color("%s: \n", DBG_IDENTIFIER, kv.first.c_str());
            const auto& vec = kv.second;
            for (int i = 0; i < vec.size(); ++i) {
                vec[i]->dbg_print();
                printf("\n");
                if(vec[i]->nested_symbol_table) {
                    vec[i]->nested_symbol_table->print_all();
                }
            }
        }
    }
    ...
    template<typename T>
    symbol_ref create_symbol(const char* name, const PP_FILE* file, bool no_reflect) {
        std::string str_name = name;
        if (str_name.empty()) {
            str_name = std::string("$unnamed_") + gen_random_string(16);
            no_reflect = true;
        }

        auto it = symbols.find(str_name);
        if (it == symbols.end()) {
            it = symbols.insert(std::make_pair(str_name, SYMBOL_ARRAY())).first;
        }
    ...
    void add_symbol(const symbol_ref& sym, bool no_reflect/*, bool pending = false*/) {
        if (sym->name.empty()) {
            sym->name = std::string("$unnamed_") + gen_random_string(16);
        }
        /*
        if (pending) {
            auto it = pending_symbols.find(sym->name);
            if (it == pending_symbols.end()) {
                it = pending_symbols.insert(std::make_pair(sym->name, SYMBOL_ARRAY())).first;
            }
            auto& sym_vec = it->second;
            sym_vec.push_back(sym);
            sort_symbols(sym_vec);
    ...
    symbol_ref get_symbol(const std::string& local_name, int lookup_flags) const {
        ...
        /*
        auto it_tmp = pending_symbols.find(local_name);
        if (it_tmp != pending_symbols.end()) {
            const auto& vec = it_tmp->second;
            for (int i = 0; i < vec.size(); ++i) {
