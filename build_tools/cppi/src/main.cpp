...
void reflect_{{ CLASS.ALT_NAME }}() {
    type_register<{{ CLASS.DECL_NAME }}>("{{ CLASS.ALIAS }}"){% if length(CLASS.OBJECTS) == 0 %};{% endif %}
{% for OBJECT in CLASS.OBJECTS %}        .prop("{{ OBJECT.ALIAS }}", &{{ OBJECT.DECL_NAME }}){% if loop.is_last %};{% endif %}
{% endfor %}
}
{% endfor %}
)";
const char* tpl_test_unity_cpp =
R"(/*
    This file was generated by a tool. Any changes made to it will be lost
*/
//#include <cppi.hpp>
{% for INCL in INCLUDE_FILES %}#include "{{ INCL }}"
{% endfor %}

{% for CLASS in CLASSES %}extern void reflect_{{ CLASS.ALT_NAME }}();
{% endfor %}

void reflect_init() {
{% for CLASS in CLASSES %}    reflect_{{ CLASS.ALT_NAME }}();
{% endfor %}
}
)";
...
bool make_reflection_template_data(
    symbol_table* scope,
    std::unordered_map<std::string, nlohmann::json>& tpl_data_map
) {
    bool has_reflected_symbols
        = !scope->types.empty()
        || !scope->functions.empty()
        || !scope->objects.empty();
    if (!has_reflected_symbols) {
        return false;
    }
    ...
    for (auto& kv : scope->types) {
        ...
        if (sym->is<symbol_class>()) {
            auto& json = tpl_data_map[sym->file->filename_canonical];
            json["HEADER_NAME"] = sym->file->file_name;
            auto& jclasses = json["CLASSES"];
            auto& jclass = jclasses.emplace_back();// [sym->global_qualified_name];
            ...
            make_reflection_template_data(sym->nested_symbol_table.get(), tpl_data_map);
        }
    }
...
void make_reflection_files(const std::string& output_dir, std::vector<translation_unit>& units, const std::string& unity_filename_override) {
    ...
    for (int i = 0; i < units.size(); ++i) {
        ...
        //make_reflection_template_data(scope.get(), tpl_data_map);
    }
