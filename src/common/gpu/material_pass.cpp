...
void gpuMaterialPass::addShaderSet(ResourceRef<gpuShaderSet> shaders) {
    shader_sets.push_back(shaders);
}

int gpuMaterialPass::shaderSetCount() const {
    return shader_sets.size();
}

gpuShaderSet* gpuMaterialPass::getShaderSet(int i) const {
