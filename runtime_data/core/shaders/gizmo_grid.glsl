...
void main(){
	...
	vec3 grid_col = vec3(luminance(bg_col.xyz) - .35);
	//vec3 grid_col = mix(vec3(1), vec3(0), luminance(bg_col.xyz));
	
	outColor = vec4(grid_col.xyz, col_frag.a * mul);
}
